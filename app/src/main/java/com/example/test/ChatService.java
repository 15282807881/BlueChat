...
public class ChatService {
    ...
    public ChatService(Context context, Handler handler){
        ...
        mState = STATE_NONE;
        mHandler = handler;  //Handler用法是传递消息
    }
    //设置当前状态
    private synchronized void setState(int state){
        ...
        mHandler.obtainMessage(MainActivity.MESSAGE_STATE_CHANGE,state,-1)
                .sendToTarget();//obtainMessage（） 获取信息，sendToTarget（） 发送信息
    }
    //获得当前状态
    ...
    private class AcceptThread extends Thread{
        ...
        public AcceptThread(){
            ...
            try{
                tmp = mAdapter.listenUsingRfcommWithServiceRecord(NAME,MY_UUID); //新建一个ServerSocket
            }catch (IOException e){}
            mmServerSocket = tmp;
        }
        ...
        public void run(){
            ...
            while(mState != STATE_CONNECTED){
                try{
                    socket = mmServerSocket.accept();//监听并返回一个
                }catch (IOException e) {
                    break;
                }
                if(socket != null){
                    connected(socket,socket.getRemoteDevice());//调用上面的个ConnectThread来管理对应的当前连接
                    try{
    ...
    private class ConnectedThread extends Thread{
        private final BluetoothSocket mmSocket;
        private final InputStream mmInStream; //文件输入流
        private final OutputStream mmOutStream;//文件输出流
